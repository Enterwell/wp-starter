...
class Events_Service {
	...
	public function on_save_event( \WP_Post $wp_post, array $r ): Event {
		...
		$event->start_date = ! empty( $r['ew_event_start_date'] ) ? new \DateTime( $r['ew_event_start_date'] ) : $event->start_date;
		$event->end_date   = ! empty( $r['ew_event_end_date'] ) ? new \DateTime( $r['ew_event_end_date'] ) : $event->start_date;
