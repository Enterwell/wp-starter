...
class Two_Factor_Email extends Two_Factor_Provider implements ITSEC_Two_Factor_Provider_On_Boardable {
	...
	public function get_label() {
		return _x( 'Email', 'Provider Label', 'better-wp-security' );
	}
	...
	public function generate_and_email_token( $user, $is_signup = false ) {
		...
		$subject = sprintf( __( '[%1$s] %2$s', 'better-wp-security' ), $url, $subject );
	...
	public function authentication_page( $user ) {
		...
		?>
		<p style="padding-bottom:1em;"><?php printf( esc_html__( 'An Authentication Code has been sent to the email address associated with your account. Look for an email with "%s" in the subject line.', 'better-wp-security' ), $subject ); ?></p>
		<p>
		...
		<?php
		submit_button( __( 'Log In', 'better-wp-security' ) );
	}
	...
	public function user_options( $user ) {
		...
		?>
		<div>
			<?php echo esc_html( sprintf( __( 'Authentication codes will be sent to %1$s.', 'better-wp-security' ), $email ) ); ?>
		</div>
		<?php
	}

	public function description() {
		echo '<p class="description">' . __( 'Time-sensitive codes are supplied via email to the email address associated with the user\'s account. Note: This WordPress site must support sending emails for this method to work (for example, sending WordPress-generated emails such as password reset and new account emails).', 'better-wp-security' ) . '</p>';
	}
	...
	public function get_on_board_label() {
		return esc_html__( 'Email', 'better-wp-security' );
	}
	...
	public function get_on_board_description() {
		return esc_html__( 'Receive an email every time you login.', 'better-wp-security' );
	}
		...
		switch ( $data['itsec_method'] ) {
			case 'verify-email-code':
				...
				if ( $this->validate_token( $user->ID, $data['itsec_email_code'] ) ) {
					...
					wp_send_json_error( array(
						'message' => esc_html__( 'The code you supplied is not valid. Please check your email for a new code.', 'better-wp-security' ),
					) );
				}
				break;
			case 'send-email-code':
				...
				wp_send_json_success( array(
					'message' => esc_html__( 'Email sent!', 'better-wp-security' ),
				) );
				break;
		}
	}
